...
public class WorldGenWhiteChocolateTree extends WorldGenAbstractTree {
	private static final String __OBFID = "CL_00000435";

	public WorldGenWhiteChocolateTree(boolean par1) {
		super(par1);
	}
	...
	public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5) {
		...
		if (par4 >= 1 && par4 + l + 1 <= 256) {
			...
			if (!flag) {
				...
				boolean isSoil = block1.canSustainPlant(par1World, par3, par4 - 1, par5, ForgeDirection.UP, (BlockCocoaSapling) PlantRegistry.cocoasapling);
				if (isSoil && par4 < 256 - l - 1) {
						...
						for (int l2 = par3 - l3; l2 <= par3 + l3; ++l2) {
							...
							for (int j3 = par5 - l3; j3 <= par5 + l3; ++j3) {
								...
								if ((Math.abs(i3) != l3 || Math.abs(k3) != l3 || l3 <= 0) && par1World.getBlock(l2, k2, j3).canBeReplacedByLeaves(par1World, l2, k2, j3)) {
									this.setBlockAndNotifyAdequately(par1World, l2, k2, j3, PlantRegistry.cocoaleaf, 3);
								}
							}
						}
					...
					for (k2 = 0; k2 < l - i4; ++k2) {
						...
						if (block2.isAir(par1World, par3, par4 + k2, par5) || block2.isLeaves(par1World, par3, par4 + k2, par5)) {
							this.setBlockAndNotifyAdequately(par1World, par3, par4 + k2, par5, BlockRegistry.chocolatepiller, 3);
						}
					}
