...
public class MapGenChocolateRavine extends MapGenBase
{
    ...
    {
        return data[index] == LiquidRegistry.chocolateMilkBlock || data[index] == LiquidRegistry.chocolateMilkBlock;
    }
    ...
    private boolean isExceptionBiome(BiomeGenBase biome)
    {
        ...
        if (biome == BiomeGenChocolateBase.chocolateDesert) return true;
        return false;
    }
    ...
    private boolean isTopBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? data[index] == BlockRegistry.chocoGrass : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        Block top    = (isExceptionBiome(biome) ? BlockRegistry.chocoGrass : biome.topBlock);
        Block filler = (isExceptionBiome(biome) ? BlockRegistry.chocoDirt : biome.fillerBlock);
        Block block  = data[index];

        if (block == BlockRegistry.chocoStone || block == filler || block == top)
        {
            if (y < 10)
            {
                data[index] = LiquidRegistry.moltenCaramelBlock;
            }
            else
            {
                data[index] = null;
