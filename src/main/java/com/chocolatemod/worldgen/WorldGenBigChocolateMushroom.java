...
public class WorldGenBigChocolateMushroom extends WorldGenerator
{
    /** The mushroom type. 0 for brown, 1 for red. */
    private int mushroomType = -1;
    private static final String __OBFID = "CL_00000415";

    public WorldGenBigChocolateMushroom(int par1)
    {
        super(true);
        this.mushroomType = par1;
    }

    public WorldGenBigChocolateMushroom()
    {
        super(false);
    }
    ...
    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
        ...
        if (par4 >= 1 && par4 + i1 + 1 < 256)
        {
            ...
            else
            {
                ...
                if (block1 != Blocks.dirt && block1 != Blocks.grass && block1 != Blocks.mycelium && block1 != BlockRegistry.chocoGrass)
                {
                    return false;
                }
                else
                {
                    ...
                    for (k1 = k2; k1 <= par4 + i1; ++k1)
                    {
                        ...
                        for (int l2 = par3 - l1; l2 <= par3 + l1; ++l2)
                        {
                            for (int i2 = par5 - l1; i2 <= par5 + l1; ++i2)
                            {
                                ...
                                {
                                    this.setBlockAndNotifyAdequately(par1World, l2, k1, i2, Block.getBlockById(Block.getIdFromBlock(PlantRegistry.chocolatemushroomcapwhite) + l), j2);
                                }
                            }
                        }
                    }
                    ...
                    for (k1 = 0; k1 < i1; ++k1)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(par1World, par3, par4 + k1, par5, Block.getBlockById(Block.getIdFromBlock(PlantRegistry.chocolatemushroomcapwhite) + l), 10);
                        }
                    }
