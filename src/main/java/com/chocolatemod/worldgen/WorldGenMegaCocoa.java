...
public class WorldGenMegaCocoa extends WorldGenHugeCocoaTrees
{
    private static final String __OBFID = "CL_00000420";

    public WorldGenMegaCocoa(boolean p_i45456_1_, int p_i45456_2_, int p_i45456_3_, int p_i45456_4_, int p_i45456_5_)
    {
        ...
        else
        {
            ...
            for (int i1 = par4 + l - 2 - par2Random.nextInt(4); i1 > par4 + l / 2; i1 -= 2 + par2Random.nextInt(4))
            {
                ...
                for (l1 = 0; l1 < 5; ++l1)
                {
                    ...
                    this.setBlockAndNotifyAdequately(par1World, j1, i1 - 3 + l1 / 2, k1, PlantRegistry.CocoaLog, 0);
                }
            ...
            for (int l2 = 0; l2 < l; ++l2)
            {
                ...
                {
                    this.setBlockAndNotifyAdequately(par1World, par3, par4 + l2, par5, PlantRegistry.CocoaLog, 0);

                    if (l2 > 0)
                    {
                        if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3 - 1, par4 + l2, par5))
                        {
                            this.setBlockAndNotifyAdequately(par1World, par3 - 1, par4 + l2, par5, PlantRegistry.stringyCaramel, 8);
                        }

                        if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3, par4 + l2, par5 - 1))
                        {
                            this.setBlockAndNotifyAdequately(par1World, par3, par4 + l2, par5 - 1, PlantRegistry.stringyCaramel, 1);
                        }
                    }
                }
                ...
                if (l2 < l - 1)
                {
                    ...
                    {
                        this.setBlockAndNotifyAdequately(par1World, par3 + 1, par4 + l2, par5, PlantRegistry.CocoaLog, 0);

                        if (l2 > 0)
                        {
                            if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3 + 2, par4 + l2, par5))
                            {
                                this.setBlockAndNotifyAdequately(par1World, par3 + 2, par4 + l2, par5, PlantRegistry.stringyCaramel, 2);
                            }

                            if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3 + 1, par4 + l2, par5 - 1))
                            {
                                this.setBlockAndNotifyAdequately(par1World, par3 + 1, par4 + l2, par5 - 1, PlantRegistry.stringyCaramel, 1);
                            }
                        }
                    }
                    ...
                    {
                        this.setBlockAndNotifyAdequately(par1World, par3 + 1, par4 + l2, par5 + 1, PlantRegistry.CocoaLog, 0);

                        if (l2 > 0)
                        {
                            if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3 + 2, par4 + l2, par5 + 1))
                            {
                                this.setBlockAndNotifyAdequately(par1World, par3 + 2, par4 + l2, par5 + 1, PlantRegistry.stringyCaramel, 2);
                            }

                            if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3 + 1, par4 + l2, par5 + 2))
                            {
                                this.setBlockAndNotifyAdequately(par1World, par3 + 1, par4 + l2, par5 + 2, PlantRegistry.stringyCaramel, 4);
                            }
                        }
                    }
                    ...
                    {
                        this.setBlockAndNotifyAdequately(par1World, par3, par4 + l2, par5 + 1, PlantRegistry.CocoaLog, 0);

                        if (l2 > 0)
                        {
                            if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3 - 1, par4 + l2, par5 + 1))
                            {
                                this.setBlockAndNotifyAdequately(par1World, par3 - 1, par4 + l2, par5 + 1, PlantRegistry.stringyCaramel, 8);
                            }

                            if (par2Random.nextInt(3) > 0 && par1World.isAirBlock(par3, par4 + l2, par5 + 2))
                            {
                                this.setBlockAndNotifyAdequately(par1World, par3, par4 + l2, par5 + 2, PlantRegistry.stringyCaramel, 4);
                            }
                        }
                    }
                }
