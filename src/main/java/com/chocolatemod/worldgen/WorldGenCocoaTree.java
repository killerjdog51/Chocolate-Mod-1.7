...
import com.chocolatemod.plants.BlockCocoaSapling;
...
public class WorldGenCocoaTree extends WorldGenAbstractTree
{
    private static final String __OBFID = "CL_00000432";

    
    public WorldGenCocoaTree(boolean par1)
    {
        this(par1, 4, 0, 0, false);
    }
    public WorldGenCocoaTree(boolean par1, int par2, int par3, int par4, boolean par5)
    {
        super(par1);
    }
    ...
    public boolean generate(World world, Random random, int x, int y, int z)
    {
        ...
        if (y >= 1 && y + l + 1 <= 256)
        {
            ...
            else
            {
                ...
                boolean isSoil = block3.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, (BlockCocoaSapling)PlantRegistry.cocoasapling);
                if (isSoil && y < 256 - l - 1)
                {
                    ...
                    for (j2 = 0; j2 < l; ++j2)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(world, k3, k2, l1, PlantRegistry.CocoaLog, 0);
                            i2 = k2;
                        }
                    }
...
//k3 is x and l1 is z and i2 is y
                    ...
                    for (j2 = -3; j2 <= 3; ++j2)
                    {
                        for (k2 = -3; k2 <= 3; ++k2)
                        {
                            if (Math.abs(j2) != 3 || Math.abs(k2) != 3)
                            {
                                ...
                                this.pod(world, k3 + 2, i2 - 1, l1 + 2);
                                this.pod(world, k3 - 1, i2 - 1, l1 - 2);
                                this.pod(world, k3 - 2, i2 - 1, l1 + 3);
                                this.pod(world, k3 + 1, i2 - 1, l1 - 1);
                                this.pod(world, k3 - 3, i2 - 1, l1);
                    ...
                    if (j2 != j3)
                    {
                        ...
                        for (l2 = k2; l2 < l && l3 > 0; --l3)
                        {
                            if (l2 >= 1)
                            {
                                ...
                                {
                                    this.setBlockAndNotifyAdequately(world, k3, i3, l1, PlantRegistry.CocoaLog, 0);
                                    i2 = i3;
                                }
                            }
    ...
    private void func_150525_a(World world, int x, int y, int z)
    {
        Block block = world.getBlock(x, y, z);

        if (block.isAir(world, x, y, z) || block.isLeaves(world, x, y, z))
        {
            this.setBlockAndNotifyAdequately(world, x, y, z, PlantRegistry.oldCocoaLeaf, 0);
        }
    }
    
    private void pod(World world, int x, int y, int z)
    {
        Block block = world.getBlock(x, y, z);

        if (block.isAir(world, x, y, z))
        {
            this.setBlockAndNotifyAdequately(world, x, y, z, PlantRegistry.CocoaPod, 0);
        }
    }
